...
class VariationalBayesDoubletGenotyperPositionSampleSpecific(object):
    def __init__(self, alpha_prior, gamma_prior, kappa_prior, G_prior, samples, state_map, X):    
...
#                 print sum((self.kappa / self.kappa.sum()) > 1e-2)
    ...
    def _update_G_d(self, data_type):
            ...
            for w in state_map:
                for (u, v) in state_map[w]:
                    if(u != s) and (v != s):
                        ...
                        doublet_diff_term[s, :, :, :] += safe_multiply(e_log_epsilon[w, :, np.newaxis, :],
        ...
        for s in state_map:
            ...
            doublet_same_term[s, :, :, :] = e_log_epsilon[w, :, np.newaxis, :] * doublet_same_term_temp[np.newaxis, :, :, :]
    ...
    def _compute_e_log_p(self):
        ...
        for data_type in self.data_types:
            for m in range(self.M[data_type]):
                gamma_prior += sum([compute_e_log_p_dirichlet(x, y) for x, y in zip(self.gamma[data_type][:, :, m], self.gamma_prior[data_type])])
    
            gamma_posterior += self._compute_e_log_p_gamma_posterior(data_type)
        
        kappa_prior = sum([compute_e_log_p_dirichlet(x, self.kappa_prior) for x in self.kappa.values()])
    ...
    def  _get_gamma_data_term(self, data_type):
        ...
        singlet_term = np.einsum('stnkm, stnkm -> stm',
        ...
        doublet_diff_term = np.einsum('stklm, stklm -> stm',
                                      doublet_diff_term_temp[np.newaxis, :, :, :],
                                      G_G[:, np.newaxis, :, :, :])

        doublet_diff_term_correction = np.einsum('stmk, stmk -> stm',
        ...
        # SxT
        doublet_same_term = np.einsum('stnkm, stnkm -> stm',
...
#     np.seterr(all='warn')
    ...
    samples = pd.Series([0 for _ in range(30)] + [1 for _ in range(70)])
     
    for i in range(10):
        np.random.seed(i)
        
        model = VariationalBayesDoubletGenotyperPositionSpecific(alpha_prior, gamma_prior, kappa_prior, G_prior, samples, state_map, sim['X'])
     
        model.fit(num_iters=10, debug=True)
     
        Z = model.Z.argmax(axis=1)
        
        v_dmm.append(v_measure_score(sim['Z'], Z))    
    
    print model.kappa[0].sum(), model.kappa[1]
