...
#         doublet_diff_term_temp = np.exp(self.log_G[:, np.newaxis, np.newaxis, :, :] + np.log(doublet_diff_term_temp[np.newaxis, :, :, :, :] + 1e-100))
        ...
        doublet_diff_term_temp = self.G[:, np.newaxis, np.newaxis, :, :] * doublet_diff_term_temp[np.newaxis, :, :, :, :]
    ...
    def _update_Z(self):
        ...
        singlet_term = self.e_log_epsilon[:, :, np.newaxis, np.newaxis] * singlet_term
        ...
        doublet_diff_term = self.e_log_epsilon[:, :, np.newaxis, np.newaxis, np.newaxis] * doublet_diff_term
        ...
        doublet_same_term = self.e_log_epsilon[:, :, np.newaxis, np.newaxis] * doublet_same_term
        
        # NxK
        doublet_same_term = doublet_same_term.sum(axis=(0, 1))
        
        doublet_same_term = self.e_log_d[1] + self.e_log_pi[np.newaxis, :] + doublet_same_term
    ...
    def _compute_e_log_p_alpha_posterior(self):
        return np.sum(self.e_log_d * self._get_alpha_data_term())
    
    def _compute_e_log_p_kappa_posterior(self):
        return np.sum(self.e_log_pi * self._get_kappa_data_term())
    
    def _compute_e_log_p_gamma_posterior(self):
        return np.sum(self.e_log_epsilon * self._get_gamma_data_term())
...
#                 g_g[s, :, :, :] += self.G[u, :, np.newaxis, :] * self.G[v, np.newaxis, :, :]
    ...
    def _get_gamma_data_term(self):
        ...
        # SxNxKxKxM
        doublet_diff_term = self.Z_1[np.newaxis, :, :, :, np.newaxis] * doublet_diff_term[:, np.newaxis, :, :, :]
        ...
        doublet_same_term = doublet_same_term[:, np.newaxis, :, :] * self.Z_1_k_k[np.newaxis, :, :, np.newaxis]
...
def compute_dirichlet_entropy(x):
    ...
    return log_gamma(x).sum() - log_gamma(a_0) + (a_0 - K) * psi(a_0) - ((x - 1) * psi(x)).sum()

def compute_discrete_entropy(log_x):
    return np.sum(np.exp(log_x) * log_x)
...
#                             [1, 98, 1],
    ...
    kappa_prior = np.ones(K) * 1e-6
    
    alpha_prior = np.array([66, 33])
